<template>
    <button class="btn active border-0 border-white position-fixed top-0 m-2 p-2" type="button"
      data-bs-target="#offcanvas-menu" data-bs-toggle="offcanvas"><svg xmlns="http://www.w3.org/2000/svg" width="1em"
        height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-music-note-list"
        style="font-size: 45px;padding-top: 0px;margin-top: 6px;margin-left: 12px;">
      ...
      <button class="btn active border-0 border-white position-fixed top-0 start-0 m-2 p-2" type="button" style="left: 10vh;"><svg xmlns="http://www.w3.org/2000/svg" width="1em"
        height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-music-note-list"
        style="font-size: 45px;padding-top: 0px;margin-top: 6px;margin-left: 12px;">
        <path d="M12 13c0 1.105-1.12 2-2.5 2S7 14.105 7 13s1.12-2 2.5-2 2.5.895 2.5 2"></path>
        <path fill-rule="evenodd" d="M12 3v10h-1V3z"></path>
    ...
    <div class="offcanvas offcanvas-start bg-body shadow-lg border-end show" tabindex="-1" data-bs-backdrop="false"
      data-bs-keyboard="false" id="offcanvas-menu" style="background-color: #e6f1fd !important;">
      <div class="offcanvas-header text-center border border-2 rounded"><svg xmlns="http://www.w3.org/2000/svg"
          width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-list-nested me-3"
        ...
        </svg><span class="fs-4">Song Queue</span><button class="btn-close" type="button" aria-label="Close"
          data-bs-dismiss="offcanvas"></button></div>
      <div
        class="offcanvas-body d-flex flex-column justify-content-between align-items-center align-items-xxl-center pt-0">
        <div class="container d-grid mt-2">
          <div class="search d-flex float-start float-md-end justify-content-xxl-center"><i
              class="fas fa-search float-start search-icon"></i><input v-model="songSearch" @keyup.enter="search"
              class="border-dark float-start float-sm-end custom-search-input" type="search"
              placeholder="Search songs here" style="margin-bottom: 0px;"></div>
          <div v-show="showRes">
            <div class="d-inline-flex justify-content-xxl-center list-groups" id="sugest-1"
              style="padding-left: 0px;width: 347px;margin-left: 0px;">
              <ul
                class="list-group border rounded border-1 border-white d-flex d-xxl-flex justify-content-xxl-start align-items-xxl-center"
                style="padding-right: 0px;width: 205.906px;border-radius: -28px;">
                <!-- add onlick and make that created() poperty as a method -->
                <li v-for="temp in song_list" class="list-group-item d-flex justify-content-between align-items-center"
                style="height: 42px; width: 295.906px; "><div  @click="()=>{temp={...temp,favorited:false}; directAddTrack(temp)}" style="overflow-x: auto; white-space: nowrap; height: inherit;" class="d-flex justify-content-start align-items-center">
                  <p class="text-center">{{temp.name}} <b class="fw-bold">by</b> {{temp.artist}}
                    </p>
                </div>
                
                  <button @click="()=>{temp={...temp,favorited:false}; queue(temp)}" data-bs-placement="top" title="Add To Queue"
                    class="btn active border-0 border-white">

                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16" style="width: 1.5em;
                  height: 1.5em;">
                      <path d="M12 13c0 1.105-1.12 2-2.5 2S7 14.105 7 13s1.12-2 2.5-2 2.5.895 2.5 2"></path>
                      <path fill-rule="evenodd" d="M12 3v10h-1V3z"></path>
                      <path d="M11 2.82a1 1 0 0 1 .804-.98l3-.6A1 1 0 0 1 16 2.22V4l-5 1z"></path>
          ...
          <div v-show="loading">
            <div class="d-inline-flex justify-content-xxl-center list-groups" id="sugest"
              style="padding-left: 0px;width: 347px;margin-left: 0px;">
              <ul
                class="list-group border rounded border-1 border-white d-flex d-xxl-flex justify-content-xxl-start align-items-xxl-center"
                style="padding-right: 0px;width: 205.906px;border-radius: -28px;">
                <li
                  class="list-group-item text-center d-flex d-xxl-flex flex-row flex-grow-1 flex-shrink-0 flex-fill justify-content-center align-items-baseline align-content-center justify-content-xl-center justify-content-xxl-center border-light-subtle"
                  style="height: 38px;width: 295.906px;">
                  <p class="d-flex d-xxl-flex justify-content-xxl-center"><span class="spinner-border spinner-border-sm"
                      role="status" style="margin-top: 5px;"></span></p>
                </li>
              </ul>
            </div>
          </div>
          <div id="empty" style="height: 48px;"></div>
          <div>
            <div>
              <div class="d-flex justify-content-between align-items-center">
              <h1 class="fs-3 mb-2">Queue</h1>  <i v-show="tracks.length !== 0" class="fa fa-times close ps-2" @click="tracks.splice(1,tracks.length-1)" title="clear queue"></i>            </div>
              <div v-show="tracks.length === 0" style="margin-top: 90px;">
                <p class="text-center">NO SONGS ADDED TO THE QUEUE :(<br>SEARCH AND ADD SONGS TO QUEUE</p>
              </div>
            </div>
            <div>
              <draggable
              tag="ul"
              :list="tracks"
              class="list-group vertiscrol w-100 w-md-75 w-lg-50 mx-auto" 
              handle=".handle"
              item-key="id"
              style="padding-right: 0px; width: 205.906px;"
            >
              <template #item="{ element, index }">
                <li 
                  class="list-group-item d-flex justify-content-center align-items-center" 
                  style="height: 42px; width: 295.906px; display: inherit;"
                >
                  <i class="fa fa-align-justify handle"></i>
                  <div class="flex-grow-1 d-flex justify-content-between align-items-center" style="width: 10vh;height: inherit;">
                    <div class="d-flex justify-content-start align-items-center" style="overflow-x: auto; white-space: nowrap; flex-grow: 1; max-width: 100%;">
                      <span class="text">{{ element.name }}</span>
                    </div>
                    <i class="fa fa-times close ps-2" @click="removeAt(index)"></i>
                  </div>
                </li>
              </template>
            </draggable>
        ...
        <div class="player__top">
          <div class="player-cover">
            <transition-group :name="transitionName">
              <!-- add id feild to tracks and change index to id here!!1111111111111111 -->
              <div class="player-cover__item" v-for="track in tracks" :key="track.id"
                v-show="track.id ===  currentTrack.id" :style="{ backgroundImage: `url(${track.cover})` }"></div>
            </transition-group>
          </div>
